...
class FeatureBiLSTMCTC(nn.Module):
    def __init__(
        ...
        # Regularizes the temporal encoder across all feature types (MobileNet / ViT).
        # This is simple feature-level dropout, complementary to SpecAugment masks.
    ...
    def forward(
        ...
        Args:
            features: (B, T, D)
            input_lengths: (B,)
            return_logits: if True, also return pre-softmax logits (B, T', V)
            return_attn: if True, also return attention weights from the encoder (B,H,T',T')
        Returns:
            log_probs: (B, T', V)
            out_lengths: (B,)
            logits (optional): (B, T', V)
        """
        ...
        if return_attn:
            enc_out, _mask, attn = self.encoder(x, out_lengths, return_attn=True)
        else:
        ...
        if return_logits and return_attn:
            return log_probs, out_lengths, logits.float(), attn
        if return_logits:
            return log_probs, out_lengths, logits.float()
        if return_attn:
            return log_probs, out_lengths, attn
